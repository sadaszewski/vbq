...
for nm=1:length(job.subjc)
    ...
    for i=1:length(outdef.warped)
        ...
        f=outdef.warped{i};
        p1= spm_imcalc_ui(strvcat(char(c1),char(f)),insert_pref(f,'p1_'),'(i1.*i2).*(i1>0.05)');
        p2= spm_imcalc_ui(strvcat(char(c2),char(f)),insert_pref(f,'p2_'),'(i1.*i2).*(i1>0.05)');
...
for nm=1:length(job.subjd)
    for i=1:length(job.subjd(nm).mp_vols)
        ...
        m_c1 = spm_select('FPList',fullfile(spm('Dir'),'toolbox','VBQ','newseg'),'mask_gm*');
        m_c2 = spm_select('FPList',fullfile(spm('Dir'),'toolbox','VBQ','newseg'),'mask_wm*');
        f =insert_pref(job.subjd(nm).mp_vols{i},'sw');
        p1= spm_imcalc_ui(strvcat(char(c1),char(f),m_c1),insert_pref(f,'p1_'),'(i1.*i2).*(i3)');
        p2= spm_imcalc_ui(strvcat(char(c2),char(f),m_c2),insert_pref(f,'p2_'),'(i1.*i2).*(i3)');
        m1=insert_pref(c1,'s');spm_smooth(c1,m1,job.fwhm);
        m2=insert_pref(c2,'s');spm_smooth(c2,m2,job.fwhm);
        n1=insert_pref(p1,'s');spm_smooth(p1,n1,job.fwhm);
        n2=insert_pref(p2,'s');spm_smooth(p2,n2,job.fwhm);
