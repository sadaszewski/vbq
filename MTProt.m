...
% $Id: MTProt.m 286 2011-12-11 17:05:56Z antoine $
...
if (flatAmap)
    nam2    = {nam2{:}, 'MTforA'};
    descrip = {descrip{:}, 'B1-uncorrected MT map for A flattening','Percent diff. MTR image (RD/BD)'};
end
Nmap    = nifti;
for ii=1:numel(nam2),
    V         = V_templ(1);
...
for p=1:dm(3),
    ...
    R1(R1<0)=0;
    tmp      = R1;
    Nmap(1).dat(:,:,p) = min(max(tmp,-2000),2000); %Truncating images to dynamic range. Originally 20000. Negative values are allowed or min(max(tmp,0),2000)
